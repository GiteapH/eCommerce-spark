 ...
 */
public class RandomForestLoss {
    static org.apache.log4j.Logger logger = Logger.getInstance();
    public void run(Dataset<Row> user, Dataset<Row> user_act, Dataset<Row> sku, Dataset<Row> rfmTag,Dataset<Row> loss, String model, int time){
        RandomForestLoss randomForestLoss = new RandomForestLoss();
        randomForestLoss.randomForestRegression(user, user_act, sku,rfmTag,loss,model,time);
    }

    public Dataset<Row> randomForestRegression(Dataset<Row> user,Dataset<Row> user_act,Dataset<Row> sku,Dataset<Row> rfmTag,Dataset<Row> loss,String modelPath,int time) {
        ...
        Dataset<Row> Info = commonUtils.joinLossAndBase_users(loss,historyInfo);
        // 按照比例拆分数据集为训练集和测试集
        ...
        if(modelPath==null) {
            model = ln.fit(train);
            try {
                model.write().overwrite().save("models/lossForecastModels/"+time);
            } catch (IOException e) {
                logger.error("模型保存失败");
            }
        }else{
            model = RandomForestRegressionModel.load("models/lossForecastModels/"+modelPath);
        }
        // 在测试集上进行预测
